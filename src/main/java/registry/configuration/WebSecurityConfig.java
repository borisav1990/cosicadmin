...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.headers().frameOptions().sameOrigin().and().authorizeRequests()
				.antMatchers("/resources/**", "/webjars/**", "/assets/**", "/dist/**").permitAll().antMatchers("/")
				.permitAll().antMatchers("/teachers/**").hasRole("TEACHER").antMatchers("/parent/**").hasRole("PARENT")
				.antMatchers("/admin/**").hasRole("ADMIN").antMatchers("/director/**").hasRole("DIRECTOR").anyRequest()
				.authenticated()
		...
		/*
		 * headers() .frameOptions().sameOrigin() .and() .antMatcher("/api/**")
		 * .authorizeRequests() .anyRequest().hasRole("ADMIN") .and() .formLogin()
